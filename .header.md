...
* In the public subnets:
    ...
    * An Internet Gateway (one per deployment, not shown)
    * Public NAT Gateways (one per subnet) for outbound traffic
...
* In the private subnets:
    ...
    * Private NAT Gateways (one per subnet) for outbound traffic
    * Auto-scaling groups for CDP [environment](https://docs.cloudera.com/management-console/cloud/environments/topics/mc-environments.html)** (one scaling group)
    * Auto-scaling groups for CDP [data lake](https://docs.cloudera.com/management-console/cloud/data-lakes/topics/mc-data-lake.html)** (one scaling group per host group, a total of five)
    * Auto-scaling groups for CDP [data hub](https://docs.cloudera.com/data-hub/cloud/index.html)*** (one scaling group per host group, total number depends on Data Hub configuration)
    * EKS clusters for CDP [data services](https://docs.cloudera.com/cdp-public-cloud/cloud/overview/topics/cdp-services.html)*** (one or more EKS clusters per Data Service)
    * One Amazon RDS for PostgreSQL multi-AZ database cluster with two instances (used by the CDP data lake)**

* Two AWS security groups (not shown) [as required by CDP](https://docs.cloudera.com/cdp-public-cloud/cloud/requirements-aws/topics/mc-aws-req-security-groups.html)
* A [cross-account role](https://docs.cloudera.com/cdp-public-cloud/cloud/requirements-aws/topics/mc-aws-req-credential.html) (not shown) and an attached cross-account policy (not shown) providing access to the AWS Cloud account from your  [CDP Management Console](https://docs.cloudera.com/management-console/cloud/overview/topics/mc-management-console.html)  
* Various IAM roles, policies and instance profiles (not shown) for configuring fine-grain permission for [cloud storage access](https://docs.cloudera.com/cdp-public-cloud/cloud/requirements-aws/topics/mc-idbroker-minimum-setup.html) and AWS compute services.
